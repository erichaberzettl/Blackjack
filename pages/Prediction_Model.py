...
with st.form("prediction_input", clear_on_submit=True, enter_to_submit=True):
    ...
    col2.number_input(label="Hand start value ", min_value=4, max_value=21, key="hand_start_value")
    ...
    if submitted:
        ...
        col3.metric("Predicted class", value=round(result[1]))
        contributions = {"Intercept": float(intercept)}
        for i, col in enumerate(X_cols):
            contributions[col.capitalize()] = st.session_state[col] * coefficients[0][i]
        ...
        fig, ax = plt.subplots()
        ax.plot(x, y)
        ax.plot(sum(list(contributions.values())), round(result[1], 4), "rx")
        ax.set_xlabel("z")
        ax.set_ylabel("σ(z)")
...
st.write("On the test data, the model achieved an accuracy of 64%. This is bad for a normal binary classifier, " \
...
st.latex(r"""
            z = -0.8681 - 0.0911*dealer\_upcard + 0.0908*hand\_start\_value 
         + 0.1848*is\_soft -1.3431*dealer\_ace + 2.3058*is\_blackjack
            """)
