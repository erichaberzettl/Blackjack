    ...
    if submitted:
        ...
        if results["p_value"] <= 0.05:
            st.markdown(f"At a significance level of $α = 0.05$ the null hypothesis can be rejected."  \
                        f"The p-value tells us that only {(results['p_value'] *100):.4f}% of samples would " \
...
"to calculate a meaningful sample size. The maximum sample size is capped at 101010 Blackjack rounds for performance reasons.")
...
st.markdown("- with 30 or less hands per strategy the assumptionw might not be fulfilled")
