...
with st.container():
    ...
    with c1:
        with st.container(border=True):
            ...
            player_no = st.slider(label=" ", min_value=1, max_value=6, value=1, key="player_no")
    ...
    for i in range(player_no):
        ...
        with st.container(border=True):
            ...
            hands = col2.selectbox("Hands per round", [1,2,3,4,5], key=f"hands_player_{i}", index=st.session_state.players[i]["hands"] -1)
            strats = ["Basic Strategy", "No Bust Strategy", "Custom"]
            strat = col3.selectbox("Strategy", strats, key=f"strat_player_{i}", index=strats.index(st.session_state.players[i]["strat"]))
            bet_size = col4.number_input("Bet per hand", min_value=1, max_value=100, key=f"bet_size_{i}", value= st.session_state.players[i]["bet_size"])
...
with col1:
    ...
    with st.container(border=True):
        ...
        st.segmented_control(" ", ["H17", "S17"], selection_mode="single", key="dealer_mode", width="stretch")

with col2:
    with st.container(border=True):
        st.markdown("Shoe size")
        st.segmented_control("Number of decks used", [1, 2, 4, 6, 8], selection_mode="single", key="shoe_size", width="stretch")


with st.container(border=True):
        st.markdown("Shuffle mode")
        shuffle_mode = st.segmented_control(" ", ["Cut-card", "Continuous"], 
                             selection_mode="single", key="shuffledf_mode", width="stretch")
